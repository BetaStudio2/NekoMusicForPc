...
public:
    ...
    /// 桥侧注入：当前播放曲目 id 与播放状态（libmpv 侧由 Dart 上报）
    void setPlayerState(int currentMusicId, bool isPlaying);
    /// 桥侧注入：账号 userId（决定发现/订阅的 accountTag；<=0 视为未登录不广播）
    void setAccountUserId(int userId);
...
private:
    ...
    int m_accountUserId = -1;
    int m_playerMusicId = 0;
